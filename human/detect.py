...
class cocoDetection:
    ...
    def __init__(self):
        self.interpreter, self.input_details, self.output_details = load_tflite_model(
            MOBILENET_SSD_PATH
        )
        ...
        for i in range(int(nums)):
            ...
            if classes[i] == person_id and scores[0][i] > 0.60:
                ...
                text_w, text_h = cv2.getTextSize(
                    display_str, cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.6, thickness=1
                )[0]

                ppl_boxes.append([x1, y1, x2, y2])
                if draw:
                    cv2.rectangle(img, (x1, y1), (x2, y2), (60, 179, 113), 4)
                    cv2.rectangle(
                        img,
                        (x1 + 2, y1 - 2),
